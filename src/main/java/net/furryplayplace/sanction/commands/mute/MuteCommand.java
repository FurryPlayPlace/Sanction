...
public class MuteCommand extends AbstractCommand {
    ...
    public MuteCommand(Sanction plugin) {
        super("mute", true, "moderation.mute");
        this.plugin = plugin;
    }
    ...
    public int execute(CommandContext<ServerCommandSource> context, ServerPlayerEntity sender, String[] args) {
        if (args.length < 2) {
            ...
            sender.sendMessage(Text.literal("/" + this.getName() + " <player> <expiration:unit> <reason>").formatted(Formatting.RED));
            return 0;
        }
        ...
        try {
            ...
            this.plugin.moderationManager().mutePlayer(target, reason, sender, formattedExpiration);
